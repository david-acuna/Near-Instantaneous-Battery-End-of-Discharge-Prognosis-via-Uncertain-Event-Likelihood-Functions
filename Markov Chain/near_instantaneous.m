...
% Exogenous inputs (current discharge) modeled as a two-state Markov Chain
T = [0.9780 0.0255; 0.0220 0.9745];     % Eq. (37): state transition kernel Markov Chain
[EigVec,~] = eig(T);                    
Ps = EigVec(:,1)./sum(EigVec(:,1));     % Eq. (38): stationary distribution
s = [9.7024; 8.3991];                   % Eqs. (35)-(36): states of the Markov Chain

% Threshold
v_cutoff = 33;  % cut-off voltage

% Uncertain event likelihood function (a.k.a. "uncertain hazard zone")
PEv = @(v,u) 1 - (1/2)*(1+erf(( v - (v_cutoff+R*u) )/(sqrt(2)*sigma_eta))); % Eq. (15): uncertain event likelihood function

% Other simulation parameters
kp = 800;   % time at which prognostics begin
kh = 3400;  % time at which prognostics finish
...
for traj=1:N                                            % simulation of trajectories
    ...
    U = s(randsrc(1,1,[(1:length(s));Ps']));            % set initial exogenous input
    for k=2:kh-kp+1
        X = state_trans(X,U);                           % sample x_{k+1} ~ p(x_{k+1}|x_k,u_k)
        U = s(randsrc(1,1,[(1:length(s));Ps']));        % sample u_{k+1} ~ p(u_{k+1})
...
if sigma_eta == 0.9
    axis([800 2200 0 3e-3])
elseif sigma_eta == 0.1
    axis([3000 3350 0 0.015])
end
...
histogram_bins = zeros(1,kh-kp+1);                	% count of frequency at each histogram bin
start_time = cputime;                               % starting time
rho_min = 1e-3;                                     % parameter criteria for stopping simulations
for traj=1:N                                        % simulation of trajectories
    X = x0;                                                             % set initial state
    ...
    for k=2:kh-kp+1
        X = state_trans(X,U);                                           % sample x_{k+1} ~ p(x_{k+1}|x_k,u_k)
        U = s(randsrc(1,1,[(1:length(s));Ps']));                        % sample u_{k+1} ~ p(u_{k+1})
        Expected_Voc = PEv(Voc(X),s(1))*Ps(1) + PEv(Voc(X),s(2))*Ps(2); % Eq. (39)
        histogram_bins(k) = histogram_bins(k) + Expected_Voc*W;         % sum contribution to histogram
        W = W*(1-PEv(Voc(X),U));                                        % update product in Eq. (26)
